...
if "total_people" not in st.session_state or st.session_state.total_people == 0:
    st.session_state.total_people = random.randint(5, 20)  # 총인원수 범위를 최대 20명으로 확장
    st.session_state.adjacent_count = random.randint(2, max(3, st.session_state.total_people - 3)) # 비율에 맞게 이웃 인원 설정
...
# 인원수가 많을(최대 20명) 때 화면이 깨지는 것을 방지하기 위해 가로 스크롤이 가능한 박스로 감싸기
seats_html = '<div style="display: flex; gap: 10px; overflow-x: auto; padding: 10px 5px;">'
for person in people_list:
    if person in selected_neighbors:
        seats_html += f"""
        <div style="flex: 0 0 auto; background-color: #FFE6EC; border: 2px dashed #FF3366; 
                    padding: 12px; border-radius: 10px; text-align: center; width: 90px;">
        ...
        seats_html += f"""
        <div style="flex: 0 0 auto; background-color: #E6F9E6; border: 1px solid #28A745; 
                    padding: 12px; border-radius: 10px; text-align: center; width: 90px;">
        ...
        """
seats_html += '</div>'
st.markdown(seats_html, unsafe_allow_html=True)
